...
import libUtils from "lib_utils";
import moleculeUtils from "molecule_utils";
import selectionTypesConstants from "selection_types_constants";
...
class Molecule3d extends React.Component {
  ...
  render3dMol() {
    ...
    const glviewer =
      this.glviewer ||
      $3Dmol.createViewer(jQuery(this.container), {
    ...
    if (
      this.props.outlineWidth !== this.lastOutline.width ||
    ...
    if (
      typeof this.props.nearClip === "number" &&
    ...
    if (!this.glviewer) {
      ...
      const intId = setInterval(() => {
        ...
        if (
          self.container.children.length > 0 &&
          self.container.children[0].offsetParent
        ) {
          glviewer.resize();
          clearInterval(intId);
        }

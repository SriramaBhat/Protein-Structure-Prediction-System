...
const moleculeUtils = {
  ...
  modelDataEquivalent(modelDataA, modelDataB) {
    ...
    const bondRelationsA = new ISet(
      modelDataA.bonds.map((bond) => `${bond.atom1_index}=>${bond.atom2_index}`)
    );
    const bondRelationsB = new ISet(
      modelDataB.bonds.map((bond) => `${bond.atom1_index}=>${bond.atom2_index}`)
    );

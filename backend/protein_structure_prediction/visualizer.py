...
def aa_to_int_seq(aaString):
  AA_ID_DICT = {'A': 1, 'C': 2, 'D': 3, 'E': 4, 'F': 5, 'G': 6, 'H': 7, 'I': 8, 'K': 9,
              'L': 10, 'M': 11, 'N': 12, 'P': 13, 'Q': 14, 'R': 15, 'S': 16, 'T': 17,
              'V': 18, 'W': 19, 'Y': 20}
  return torch.tensor([AA_ID_DICT[aa] for aa in aaString])
...
def plotting(aaString, config, device):
    aa_int_seq = aa_to_int_seq(aaString)
    ...
    print("date_string")
    sb_preds.to_pdb(date_string,path='./plots/{}_pred.pdb'.format(date_string))
